...
(function(){
  ...
  function goalLabel(goal){
    return ({'fat-loss':'Fat loss','muscle-gain':'Muscle gain',strength:'Strength','general-fitness':'General fitness'})[goal]||goal;
  }
  async function start(){
    ...
    shade.innerHTML='<div class="card" style="max-width:560px;margin:6vh auto;padding:28px"><p class="eyebrow">CLIENT ONBOARDING</p><h2>Let’s build your starting profile</h2><p class="muted">Answer these basics before your assessment. Your coach will review the information before assigning a plan.</p><form id="raf-onboarding-form" class="form-grid"><label>Age<input name="age" type="number" min="13" max="100" required></label><label>Primary goal<select name="goal" required><option value="" selected disabled>Choose your goal</option><option value="fat-loss">Fat loss</option><option value="muscle-gain">Muscle gain</option><option value="strength">Strength</option><option value="general-fitness">General fitness</option></select></label><label>Training experience<select name="experience" required><option value="" selected disabled>Choose experience</option><option>beginner</option><option>intermediate</option><option>advanced</option></select></label><label>Training days per week<input name="days" type="number" min="1" max="7" value="3" required></label><label>Equipment<select name="equipment" required><option value="gym">Gym</option><option value="home">Home</option><option value="none">No equipment</option></select></label><label>Anything your coach should know?<textarea name="notes" rows="3"></textarea></label><button class="primary" type="submit">Save and continue →</button><p id="raf-onboarding-status" class="muted"></p></form></div>';
    ...
    shade.querySelector('form').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget,d=Object.fromEntries(new FormData(form));if(!d.goal||!d.experience){form.reportValidity();return}d.age=Number(d.age);d.days=Number(d.days);const st=shade.querySelector('#raf-onboarding-status'),btn=form.querySelector('button');btn.disabled=true;st.textContent='Saving...';try{const remote=await saveRemote(s,d);cache(s,remote||d);shade.remove();location.hash='client-portal';window.render?.('client-portal')}catch(err){console.error(err);st.textContent='Could not save: '+(err.message||'please try again');btn.disabled=false}};
  }
