...
(function(){
 ...
 function go(view){location.hash=view; if(typeof window.render==='function') window.render(view); window.dispatchEvent(new Event('hashchange'));}
 function renderPortal(){
  ...
  root.querySelector('#client-checkin-form')?.addEventListener('submit',async e=>{
   ...
   try{
    ...
    const out={weight:Number(f.elements.weight.value),height:Number(f.elements.height.value),waist:Number(f.elements.waist.value),notes:f.elements.notes.value||'',date:new Date().toISOString()};
    ...
    const r=await db().from('client_checkins').insert({client_id:uid,weight:out.weight,height:out.height,waist:out.waist,front_photo:out.front||null,side_photo:out.side||null,back_photo:out.back||null,notes:out.notes});
