...
(function(){'use strict';
  ...
  function ensureCards(){
    const builder=$('#meal-builder'); if(!builder) return;
    let box=$('#raf-remaining-cards');
    if(!box){
      box=document.createElement('section');
      box.id='raf-remaining-cards';
      box.className='card';
      box.innerHTML='<p class="eyebrow">REMAINING TARGETS</p><div class="raf-rem-grid"></div>';
      // Keep the cards at the very bottom, after all meal fields and the save button.
      builder.parentNode.appendChild(box);
    }
    update();
  }
  function update(){
    ...
    document.querySelectorAll('#meal-builder .meal-row').forEach(row=>{
      ...
      const f=(window.RAF_FOODS||[])[Number(sel.value)], g=Number(inp.value)||0;
      if(!f||g<=0)return;
      const q=g/100;
    ...
    box.querySelector('.raf-rem-grid').innerHTML=data.map(([k,l,u])=>{
      const rem=Math.max(0,target[k]-totals[k]);
      return `<div class="raf-rem-card"><span>${l} remaining</span><strong>${num(rem)} <small>${u}</small></strong><em>Used ${num(totals[k])} / ${target[k]||0} ${u}</em></div>`;
    }).join('');
  }
  const style=document.createElement('style');
  style.textContent='#raf-remaining-cards{margin:16px 0 24px}.raf-rem-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.raf-rem-card{padding:14px;border:1px solid var(--border,#ddd);border-radius:14px;background:var(--surface,#fff)}.raf-rem-card span,.raf-rem-card em{display:block;font-size:.8rem;color:var(--muted,#777)}.raf-rem-card strong{display:block;font-size:1.45rem;margin:6px 0}.raf-rem-card small{font-size:.75rem}.raf-rem-card em{font-style:normal}@media(max-width:700px){.raf-rem-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}';
  document.head.appendChild(style);
  const old=window.render;
  window.render=function(v){const r=old&&old.apply(this,arguments);if(v==='nutrition')setTimeout(ensureCards,0);return r;};
  document.addEventListener('input',e=>{if(e.target.closest('#meal-builder')){ensureCards();update();}});
  document.addEventListener('change',e=>{if(e.target.closest('#meal-builder')){ensureCards();update();}});
