/* RAF Coaching — role boundary guard v2 */
(function(){
  ...
  function applyClientNav(){
    if(read('rafSession',{}).role!=='client') return;
    document.querySelectorAll('.nav-item').forEach(btn=>{
      const v=btn.dataset.view;
      if(coachOnly.has(v)) btn.remove();
    });
  }
  const oldRender=window.render;
  window.render=function(view){
    const s=read('rafSession',{});
    if(s.role==='client' && coachOnly.has(view)){
      view='client-portal';
    }
    if(s.role==='client' && view==='nutrition'){
      ...
      setTimeout(()=>{
        ...
        if(select){const label=select.closest('label');if(label) label.remove();else select.remove();}
        const title=document.querySelector('#app-content .hero h2');
        if(title) title.textContent=(p.name||'Client')+' · Nutrition';
      },0);
      return;
    }
    const result=oldRender?.(view);
    setTimeout(applyClientNav,0);
    return result;
  };
  document.addEventListener('DOMContentLoaded',()=>setTimeout(applyClientNav,120));
})();
