(function(){
  ...
  async function submit(mode){
    ...
    await client.from('profiles').upsert({id:u.id,full_name:name||u.user_metadata?.full_name||email.split('@')[0],role:role||u.user_metadata?.role||'client'});
    localStorage.setItem('rafSession',JSON.stringify({id:u.id,name:name||email,role,createdAt:Date.now(),supabase:true}));
    location.reload();
  }
  async function boot(){
    if(!window.supabase)return;
    client=window.supabase.createClient(SUPABASE_URL,SUPABASE_KEY); window.rafSupabase=client;
    const {data}=await client.auth.getSession();
    if(data.session){localStorage.setItem('rafSession',JSON.stringify({id:data.session.user.id,name:data.session.user.email,role:'client',supabase:true}));document.getElementById('raf-auth-overlay')?.remove();document.getElementById('raf-supa-auth')?.remove();}
    else show();
  }
