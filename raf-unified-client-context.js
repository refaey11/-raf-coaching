...
(function(){
  ...
  function ensure(){
    ...
    if(p){let match=list.find(c=>c.id===p.id)||(list.find(c=>c.name===p.name));if(match&&match!==p){p={...match,...p,id:match.id};write('rafProfile',p)}}
    return {list,profile:p};
  }
  function active(){return ensure().profile}
  function patchNutrition(){
    const {list,profile}=ensure();
    const sel=document.querySelector('#raf-nutrition-client');
    if(sel){
      [...sel.options].forEach((o,i)=>{if(list[i])o.value=list[i].id});
      if(profile){const i=list.findIndex(c=>c.id===profile.id);if(i>=0)sel.value=profile.id}
      sel.onchange=()=>{const c=list.find(x=>x.id===sel.value);if(c){write('rafProfile',c);location.hash='nutrition';if(typeof window.render==='function')window.render('nutrition');setTimeout(patchNutrition,30)}};
    }
  ...
  function patchProfileLinks(){
    document.querySelectorAll('[data-client]').forEach(btn=>{if(btn.dataset.contextPatched)return;btn.dataset.contextPatched='1';btn.addEventListener('click',()=>{const {list}=ensure(),c=list[+btn.dataset.client];if(c)write('rafProfile',c)},true)});
  }
