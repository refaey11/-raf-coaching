...
(function(){
  ...
  function normalize(plan){
    ...
    plan.days.forEach((d,i)=>{d.name=d.name||`Day ${i+1}`;d.focus=d.focus||'Program session';d.exercises=Array.isArray(d.exercises)?d.exercises:[]});
    plan.variables=plan.variables||{};
    plan.variables.sets=plan.variables.sets??3;
    plan.variables.reps=plan.variables.reps??'10-12';
    plan.variables.tempo=plan.variables.tempo??'2/0/2';
    plan.variables.rest=plan.variables.rest??'60 sec';
    return plan;
  }
  function workout(){
    ...
    const selected=Math.max(0,Math.min(Number(sessionStorage.getItem('rafWorkoutDay')||0),plan.days.length-1)), day=plan.days[selected],v=plan.variables;
    ...
    return `<div class="hero"><p class="eyebrow">NASM / OPT WORKOUT</p><h2>${esc(p.name)} · Phase ${esc(plan.phase??'1')}</h2><p class="muted">${esc(plan.phaseName??'Stabilization Endurance')} · ${esc(p.goal??'')}</p></div><div class="card"><h3>Training days</h3><div style="display:flex;gap:8px;flex-wrap:wrap">${tabs}</div><p class="muted">${esc(day.focus)}</p></div><div class="card"><h3>Session settings</h3><div class="form-grid"><label>Sets<input id="wo-sets" type="number" min="1" value="${esc(v.sets)}"></label><label>Reps<input id="wo-reps" value="${esc(v.reps)}"></label><label>Tempo<input id="wo-tempo" value="${esc(v.tempo)}"></label><label>Rest<input id="wo-rest" value="${esc(v.rest)}"></label></div></div><div class="card"><h3>${esc(day.name)} · Exercise sequence</h3>${ex.length?ex.map((e,i)=>`<article class="workout" style="display:flex;gap:16px;align-items:flex-start;margin:16px 0"><img src="${visual(e)}" alt="${esc(e.name)}" style="width:150px;height:90px;object-fit:cover;border-radius:12px"><div style="flex:1"><strong>${i+1}. ${esc(e.name)}</strong><div class="muted">${esc((e.muscles||[]).join(' · '))} · ${esc(e.equipment||'Bodyweight')}</div><p>${esc(e.instruction||'Controlled technique. Maintain alignment and breathing.')}</p><div class="muted">${esc(e.sets??v.sets)} sets · ${esc(e.reps??v.reps)} reps · ${esc(e.rest??v.rest)}</div><label>Completed <input type="checkbox" data-done="${i}"></label></div></article>`).join(''):'<p class="muted">No exercises in this day yet. Add exercises from Program Builder.</p>'}</div><button class="primary" id="save-workout">Save workout session</button><p id="wo-status" class="muted"></p>`;
  }
  function bind(){
